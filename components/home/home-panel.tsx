...
const EventMap = dynamic(
  () => import('@/components/event-map').then((m) => ({ default: m.EventMap })),
  { ssr: false },
)
...
/**
 * HomePanel — the entire home UI for a SINGLE event.
 *
 * Everything that used to live directly in `app/(app)/home/page.tsx` now
 * lives here, parametrised by `event` so we can mount one panel per
 * available event and swap between them on swipe.
 *
 * Per-panel state (announcements, schedule, weather, attendee count,
 * checklist) is scoped to `event.id` so each panel maintains its own
 * fetched data. Global state — user, profile — still comes from auth
 * context.
 */
export function HomePanel({ event, venue }: HomePanelProps) {
  const { user, profile } = useAuth()
  const { ticketsByEvent } = useHomeEvent()
  const qrCode = ticketsByEvent[event.id] ?? null
  ...
  // Reset hero-error state whenever we mount a new panel / image
  useEffect(() => {
    setHeroFailed(false)
  }, [heroImageUrl])
  ...
  const [schedule, setSchedule] = useState<
    { id: string; title: string; start_time: string; end_time: string | null; icon: string }[]
  >([])
  const [weather, setWeather] = useState<{ max: number; min: number; code: number } | null>(null)
  const [checks, setChecks] = useState<Record<string, boolean>>({})

  // ── Announcements (event-scoped + venue general) ─────────────────────────
  useEffect(() => {
    if (!event.id) return
    let cancelled = false
    ...
    let channel = supabase
      .channel(`home-announcements-${event.id}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  // ── Drink order + schedule + attendee count (event-scoped) ───────────────
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        .from('event_schedule')
        .select('id, title, start_time, end_time, icon')
        .eq('event_id', event.id)
        .order('start_time', { ascending: true }),
    ]).then(([drinkRes, scheduleRes]) => {
      if (cancelled) return
  ...
  // ── Weather forecast for event day (OpenMeteo) ───────────────────────────
  useEffect(() => {
    ...
    fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${venue.latitude}&longitude=${venue.longitude}&daily=temperature_2m_max,temperature_2m_min,weathercode&timezone=Europe/Madrid&start_date=${dateStr}&end_date=${dateStr}`,
    )
      .then((r) => r.json())
      .then((data) => {
        if (data.daily) {
  ...
  // ── Checklist (localStorage, per-event) ──────────────────────────────────
  useEffect(() => {
    if (!event.id) return
    try {
  ...
  // Stable timestamp for timeAgo formatting
  ...
  // Build the single-ticket array for the panel's QR carousel (preserves the
  // fullscreen-tap affordance while avoiding showing siblings that belong to
  // other panels).
  const panelTickets: CarouselTicket[] = qrCode
    ? [
        {
          id: event.id,
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="relative rounded-2xl overflow-hidden" style={{ height: '240px' }}>
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-5 animate-blur-in">
          <p className="text-white-muted text-sm">
            Hola,{' '}
            <span className="text-accent-gradient font-semibold">{firstName}</span> 👋
          </p>
      ...
      {/* Aftermovie */}
      {(event as unknown as { video_url?: string | null }).video_url &&
        (() => {
          const url = (event as unknown as { video_url: string }).video_url
            ...
            <div className="card-glow overflow-hidden animate-slide-up">
              ...
              {ytId ? (
                ...
                >
                  ...
                  <p className="text-center text-xs text-white-muted pb-4">
                    Toca para ver el aftermovie
                  </p>
                </a>
              )}
            </div>
      ...
      {(() => {
        const hoursUntil = event.date
          ? (new Date(event.date).getTime() - Date.now()) / 3600000
          : Infinity
        ...
        const items = [
          { id: 'dni', label: 'DNI / Documento de identidad', sub: 'Imprescindible para entrar' },
          {
            id: 'entrada',
            label: 'Entrada',
            sub: qrCode ? 'Ya tienes tu QR listo' : 'Asegurate de tener tu entrada',
            auto: !!qrCode,
          },
          {
            id: 'autorizacion',
            label: `${authLabel} (menores)`,
            sub: 'Impresa y firmada por padre/madre/tutor',
          },
          {
            id: 'fotocopia',
            label: 'Fotocopia DNI del padre/madre/tutor',
            sub: 'Del que firme la autorizacion',
          },
        ]
          ...
          <div className="card-glow p-5 space-y-4 animate-slide-up animate-glow-pulse">
            ...
            <div className="p-3 rounded-xl bg-yellow-500/[0.07] border border-yellow-500/20">
              ...
              <p className="text-[11px] text-white-muted leading-relaxed">
                El DNI, la autorizacion y la fotocopia tienen que estar{' '}
                <span className="text-white font-medium">impresos en papel</span>. No se aceptan fotos ni documentos en el movil.
              </p>
            </div>
      ...
      {/* Ticket card (or complete-order banner) */}
      {panelTickets.length > 0 ? (
        <QRCarousel tickets={panelTickets} userName={profile?.full_name || ''} />
      ) : !hasDrinkOrder ? (
        <Link
          href="/polls"
          className="card-glow p-4 flex items-center gap-3 active:scale-[0.98] transition-transform animate-glow-pulse"
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-white-muted">
              Elige tus bebidas para obtener tu entrada
            </p>
          </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        <Link
          href="/polls"
          className="card-glow p-3.5 text-center active:scale-[0.92] transition-transform animate-scale-in"
        >
          <div
          ...
          <p className="text-[11px] font-medium text-white">
            {hasDrinkOrder ? 'Pedido' : 'Bebidas'}
          </p>
        </Link>
        <Link
          href="/gallery"
          className="card-glow p-3.5 text-center active:scale-[0.92] transition-transform animate-scale-in delay-100"
        >
        ...
        <Link
          href="/chat"
          className="card-glow p-3.5 text-center active:scale-[0.92] transition-transform animate-scale-in delay-200"
        >
        ...
        <Link
          href="/playlist"
          className="card-glow p-3.5 text-center active:scale-[0.92] transition-transform animate-scale-in delay-300"
        >
      ...
      {announcements.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link
              href="/chat"
              className="text-[11px] text-white-muted flex items-center gap-0.5 hover:text-white transition-colors"
            >
          ...
          <div className="space-y-2">
            {announcements.map((a, i) => (
              <div
                key={a.id}
                className={cn(
                  'card p-4 animate-slide-right',
                  a.is_general
                    ? 'border-violet-400/15 border-l-2 border-l-violet-400/40'
                    : 'border-primary/10 border-l-2 border-l-primary/30',
                )}
              ...
              >
                ...
                <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-white/5">
                  <div className="flex items-center gap-2">
                    ...
                    {a.is_general && (
                      <span className="text-[9px] font-medium text-violet-400 bg-violet-400/10 px-1.5 py-0.5 rounded-full">
                        General
                      </span>
                    )}
                  </div>
        ...
        {(event.location || venue?.address) && (
          <div className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {venue?.city && (
                <p className="text-[11px] text-white-muted truncate">{venue.city}</p>
              )}
            </div>
          </div>
        )}
            ...
            <div className="card p-4 flex items-center gap-4 animate-slide-up">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-white-muted">
                  {w.label} · Max {weather.max}° / Min {weather.min}°
                </p>
              </div>
        ...
        <div>
          ...
          <div className="space-y-0">
            {schedule.map((item, i) => {
              ...
              const isCurrent = nowDate >= start && (!end || nowDate <= end)
              const isPast = end
                ? nowDate > end
                : schedule[i + 1]
                ? nowDate > new Date(schedule[i + 1].start_time)
                : false
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <a
            href={`https://m.uber.com/ul/?action=setPickup&pickup[latitude]=${
              venue?.latitude || ''
            }&pickup[longitude]=${venue?.longitude || ''}&pickup[nickname]=${encodeURIComponent(
              venue?.name || 'Venue',
            )}`}
            target="_blank"
            rel="noopener noreferrer"
      ...
      <div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-white-muted text-[11px] mt-0.5 leading-snug">
              Anade otra graduacion o fiesta
            </p>
          </div>
          <ChevronRight className="w-4 h-4 text-white-muted flex-shrink-0" />
        </Link>
      ...
      {/* Social */}
      <div>
        <h2 className="text-xs font-semibold uppercase tracking-widest text-white-muted mb-3">
          Siguenos
        </h2>
        <div className="grid grid-cols-2 gap-2.5">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="card flex items-center gap-3 p-3.5 active:scale-95 transition-all hover:shadow-[0_0_20px_rgba(131,58,180,0.15)]"
          >
            <div
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="card flex items-center gap-3 p-3.5 active:scale-95 transition-all hover:shadow-[0_0_20px_rgba(255,255,255,0.08)]"
          >
